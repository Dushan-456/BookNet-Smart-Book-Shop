...
const ProductsPage = () => {
   ...
   const fetchProducts = useCallback(async () => {
      ...
      if (loading && page > 1) {
         // This `loading` refers to the state from the closure, before current fetch might change it
   ...
   if (loading && page === 1 && products.length === 0) {
      // Add products.length === 0 to ensure it only shows if no products yet
      return (
         <Box
            sx={{
               display: "flex",
   ...
   return (
      <div className="container mx-auto px-4 py-8">
         ...
         {loading &&
            page > 1 && ( // Show inline loading indicator for "Load More" when new page is being fetched
         ...
         {!loading &&
            hasMore && ( // Only show "Load More" if there are more products and not currently loading
         ...
         {!loading &&
            !hasMore &&
            products.length > 0 && ( // Message when all products are loaded
               <Typography
                  variant="body2"
                  color="textSecondary"
                  align="center"
